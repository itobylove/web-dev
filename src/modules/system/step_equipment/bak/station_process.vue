...
const vData=reactive({
  ...
  selectOptions: {status:[],station_id:[],process_id:[]},
})
...
const tableEvent = {
  add: async () => {
    ...
    const process_ids = await dialog.selectAsync(options,'','可添加的工艺('+getPlant(vData.plant_id)+' '+vData.station_name+')', {width:'800px',height:'200px'},{
      multiple: true,placeholder: '请输入名称过滤',clearable:true,minCollapsedNum:10,
    })
    if (!process_ids)  return;
    const loading = dialog.loading(undefined, '保存中...');
    const res = await api.post(api.url2.sys.station.process.add,{station_id:vData.station_id,process_ids:[...process_ids,...existIds]});
  ...
  delete: async (rows) => {
    ...
    const res = await api.post(api.url2.sys.station.process.del, {sid});
    loading && loading.close();
    if (res){
      dialog.success('删除成功');
      await tableRef.value.reportConfig.getData({where:{station_id:vData.station_id}});
...
const initTable = async () => {
  const apiData = vData.apiData = await api.get(api.url2.sys.station.process.config);
  ...
  table.value={
    ...
    searchConfig:false,
    tableConfig: {
      url: api.url2.sys.station.process.list,
      showCheck: true,
      disablePage: true,
...
watch(() => [props.station], async () => {
  ...
  vData.plant_id =  props.station.plant_id;
  tableRef.value.reportConfig.getData({where:{station_id:vData.station_id}});
});
