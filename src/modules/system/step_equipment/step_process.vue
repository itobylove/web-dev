...
import processPage from "@/modules/system/step_equipment/process.vue";

const props = defineProps({
  step:{type:Object,default:null,required: false},
...
const vData=reactive({
  ...
  selectOptions: {status:[],step_id:[],process_id:[]},
  ...props.query
})
...
const tableEvent = {
  add: async () => {
    ...
    const options = vData.selectOptions.process_id.filter((item) => item.data.plant_id === vData.plant_id && !existIds.includes(item.value) );
    const newIds = await dialog.selectAsync(options,'','可添加的工艺('+getPlant(vData.plant_id)+' '+vData.step_name+')', {width:'800px',height:'200px'},{
      multiple: true,placeholder: '请输入名称过滤',clearable:true,minCollapsedNum:10,
    })
    if (!newIds)  return;
    const loading = dialog.loading(undefined, '保存中...');
    const res = await api.post(api.url2.sys.step.process.add,{step_id:vData.step_id,process_ids:[...newIds,...existIds]});
    loading && loading.close();
    if (res){
      dialog.success('保存成功');
      await tableRef.value.reportConfig.getData({where:{step_id:vData.step_id}});
    }
  },
  delete: async (rows) => {
    ...
    const res = await api.post(api.url2.sys.step.process.del, {sid});
    loading && loading.close();
    if (res){
      dialog.success('删除成功');
      await tableRef.value.reportConfig.getData({where:{step_id:vData.step_id}});
  ...
  showList: async (args={}) => {
    ...
    dialog.window(processPage, args,{width: '80%',height: '80%',title:"工艺管理"})
   }
};
...
const initTable = async () => {
  const apiData = vData.apiData = await api.get(api.url2.sys.step.process.config);
  ...
  table.value={
    ...
    searchConfig:false,
    tableConfig: {
      url: api.url2.sys.step.process.list,
      showCheck: true,
      disablePage: true,
      ...
      events:{
        ...
        click_cell: async ({originData,field}) => {
          if (!field) return ;
          emit('clickCell', {type: 'process', data: originData});
        }
      },
      ...props.tableConfig,
      afterLoaded:()=>{
...
watch(() => [props.step], async () => {
  vData.step_id = props.step?.id;
  vData.step_name = props.step?.name;
  props.step?.plant_id && (vData.plant_id =  props.step?.plant_id);
  tableRef.value.reportConfig.getData({where:{step_id:vData.step_id}});
});
