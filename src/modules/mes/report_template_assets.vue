    ...
    <dialogComponent v-if="dialogConfigE.isShow" v-bind="dialogConfigE.bind"  v-on="dialogConfigE.on"  >
      <t-form :data="dialogConfigE.data" :rules="dialogConfigE.rules" @submit="dialogConfigE.submit" v-bind="dialogConfigE.form">
        <t-col span="11" >
          ...
          <t-form-item label="设备" name="assets_id" >
            <t-select v-model="dialogConfigE.data.assets_id" reserveKeyword  :options="[{label:'全选',checkAll:true},...vData.selectOptions.assets_id]"  :filterable="dialogConfigE.type === 'addAssets'" :multiple="dialogConfigE.type === 'addAssets'" :disabled="dialogConfigE.type == 'editAssets'"  />
          </t-form-item>
...
const vData=reactive({
  template_id:null,
  template_name:null,
  selectOptions: {status:[],assets_id:[],template_id:[],multiple:[]},
})
...
const mainReportConfig = {
  menuConfig: {
    ...
    menu: {
      create: { click: () => fn.addTemplateAssets()},
      delete: { click: () => fn.delTemplateAssets()},
      update: { click: () => fn.editTemplateAssets()},
    },
  },
  searchConfig:[],
  tableConfig: {
    url: apiUrl.mes.report_template.assetsList,
    showCheck: true,
    disablePage: true,
    ...
    events:{
      dblclick_cell: async ({originData,field}) => {
        if (!field) return ;
        await fn.editTemplateAssets([originData]);
      },
    }
  }
}
...
const fn = {
  addTemplateAssets: () => {
    ...
    dialogConfigE.type = 'addAssets';
    dialogConfigE.isShow = true;
  },
  editTemplateAssets: async (rows) => {
    ...
    dialogConfigE.type = 'editAssets';
    dialogConfigE.data = {...row};
    dialogConfigE.isShow = true
  },
  delTemplateAssets: async () => {
    ...
    if (!result) return;
    await api.post(apiUrl.mes.report_template.delAssets, { ids: checkedRows.map(item => item.id) }).then(
        res => {
          if (res) {
            dialog.success('删除成功');
...
const dialogConfigE = reactive({
  ...
  rules:{
    ...
    assets_id: [{required: true, message: '请选择设备', trigger: 'change'},],
...
onMounted( () => {
  ...
  api.get(apiUrl.mes.report_template.assetsConfig).then(res => {
