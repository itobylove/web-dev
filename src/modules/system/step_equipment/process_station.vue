...
import stationPage from "@/modules/system/step_equipment/station.vue";
...
const vData=reactive({
  ...
  selectOptions: {status:[],process_id:[],station_id:[]},
  ...props.query
})
...
const tableEvent = {
  add: async () => {
    ...
    const newIds = await dialog.selectAsync(options,'','可添加的工段('+getPlant(vData.plant_id)+' '+vData.process_name+')', {width:'800px',height:'200px'},{
      multiple: true,placeholder: '请输入名称过滤',clearable:true,minCollapsedNum:10,
    })
    if (!newIds)  return;
    const loading = dialog.loading(undefined, '保存中...');
    const res = await api.post(api.url2.sys.process.station.add,{process_id:vData.process_id,station_ids:[...newIds,...existIds]});
  ...
  delete: async (rows) => {
    ...
    const res = await api.post(api.url2.sys.process.station.del, {sid});
    loading && loading.close();
    if (res){
      dialog.success('删除成功');
      await tableRef.value.reportConfig.getData({where:{process_id:vData.process_id}});
  ...
  showList: async (args={}) => {
    ...
    dialog.window(stationPage, args,{width: '80%',height: '80%',title:"工段管理"})
  }
};
...
const initTable = async () => {
  ...
  table.value={
    ...
    searchConfig:false,
    tableConfig: {
      url: api.url2.sys.process.station.list,
      showCheck: true,
      disablePage: true,
      ...
      events:{
        ...
        click_cell: async ({originData,field}) => {
          if (!field) return ;
          emit('clickCell', {type: 'station', data: originData});
        }
      },
      ...props.tableConfig,
      afterLoaded:()=>{
...
watch(() => [props.process], async () => {
  ...
  props.process?.plant_id && (vData.plant_id = props.process?.plant_id);
