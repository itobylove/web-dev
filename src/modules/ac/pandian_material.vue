...
const getOptions = () => {
  dialogConfig.warehouseOptions = dialogConfig.warehouseOptionsAll.filter(item => parseInt(item.plantsId) === parseInt(dialogConfig.plantid) && parseInt(item.material) === 1);
  dialogConfig.warehouseid = dialogConfig.warehouseOptions.map(item => item.value);
}
const fn = {
  createPandian: () => {
    dialogConfig.config.title = '创建盘点表';
    dialogConfig.type = 'material_create';
    dialogConfig.show = true;
  },
  sendPandian: () => {
    dialogConfig.config.title = '推送盘点表';
    dialogConfig.type = 'material_send';
    dialogConfig.show = true;
  }
}
...
const mainReportConfig = {
  ...
  searchConfig: {
    search: [
      ...
      {
        type: 'radio', field: 'suffix', value: '-WLSYS',
        options: {placeholder: '类别', options: [{label: '仓库', value: '-WLSYS'}, {label: '财务', value: '-WLAC'}]}
      },
      {type: 'month', field: 'ym', value: core.date.Ym(), options: {placeholder: '月份'}},
    ],
  },
  tableConfig: {
    url: apiUrl.ac.pandian.material_list,
    showCheck: false,
    disablePage: false,
  }
}
...
onMounted(async () => {
  ...
  getOptions();
  api.get(apiUrl.ac.pandian.material_config).then(res => {
