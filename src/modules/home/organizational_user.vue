...
const tableEvent = {
  init: async function () {
    ...
    const apiData = await api.get(apiUrl2.home.organizational.init);
  ...
  ceshi: async (rows) => {
    // await api.post(apiUrl2.home.organizational.user_list, {})
    await api.get(api.url.system.user.list,{},'list');
  },
};
...
const getMenus = () => {
  const menus = {
    ...
    'position':{
      ...
      // assignment: {sort: 100, title: '用户岗位', icon: 'ri-user-location-line', listAction: tableEvent.assignment},
      // copyAssignment: {sort: 110, title: '测试', icon: 'ri-file-copy-line', listAction: tableEvent.ceshi},
    }
  };
...
const getReportConfig =  () => {
  return {
    dataFetcher: async (url,query) => {
      ...
      return await api.get(apiUrl2.home.organizational.user_list,{...query,auth_item:props.query.auth_item},'list');
    },
