...
const vData=reactive({
  ...
  selectOptions: {status:[],equipment_id:[],station_id:[]},
})
...
const tableEvent = {
  add: async () => {
    ...
    const options = vData.selectOptions.station_id.filter((item) => item.data.plant_id === vData.plant_id && !existIds.includes(item.value) );
    const station_ids = await dialog.selectAsync(options,'','可添加的工段('+getPlant(vData.plant_id)+' '+vData.equipment_name+')', {width:'800px',height:'200px'},{
      multiple: true,placeholder: '请输入名称过滤',clearable:true,minCollapsedNum:10,
    })
    if (!station_ids)  return;
    const loading = dialog.loading(undefined, '保存中...');
    const res = await api.post(api.url2.sys.equipment.station.add,{type:vData.type,equipment_id:vData.equipment_id,station_ids:[...station_ids,...existIds]});
    loading && loading.close();
    if (res){
      dialog.success('保存成功');
      await tableRef.value.reportConfig.getData({where:{equipment_id:vData.equipment_id}});
    }
  },
  delete: async (rows) => {
    ...
    const res = await api.post(api.url2.sys.equipment.station.del, {sid});
    loading && loading.close();
    if (res){
      dialog.success('删除成功');
      await tableRef.value.reportConfig.getData({where:{equipment_id:vData.equipment_id}});
...
const initTable = async () => {
  const apiData = vData.apiData = await api.get(api.url2.sys.equipment.station.config);
  ...
  table.value={
    ...
    searchConfig:false,
    tableConfig: {
      url: api.url2.sys.equipment.station.list,
      showCheck: true,
      disablePage: true,
...
watch(() => [props.equipment], async () => {
  ...
  vData.equipment_name = props.equipment?.name;
  vData.plant_id = props.equipment?.plant_id;
  vData.type = props.equipment?.type;
  tableRef.value.reportConfig.getData({where: {equipment_id: vData.equipment_id}});
});
