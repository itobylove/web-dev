...
import equipmentPage from "@/modules/system/step_equipment/equipment.vue";
...
const vData=reactive({
  ...
  selectOptions: {status:[],station_id:[],equipment_id:[]},
  ...props.query
})
...
const tableEvent = {
  add: async () => {
    ...
    const res = await api.post(api.url2.sys.station.equipment.add,{type:vData.type,station_id:vData.station_id,equipment_ids:[...newIds,...existIds]});
  ...
  delete: async (rows) => {
    ...
    const res = await api.post(api.url2.sys.station.equipment.del, {sid});
    loading && loading.close();
    if (res){
      dialog.success('删除成功');
      await tableRef.value.reportConfig.getData({where:{station_id:vData.station_id}});
  ...
  showList: async (args={}) => {
    args.query= {plant_id:vData.plant_id,type:vData.type};
    dialog.window(equipmentPage,args,{width: '80%',height: '80%',title:"设备管理"})
  }
};
...
const initTable = async () => {
  const apiData = vData.apiData = await api.get(api.url2.sys.station.equipment.config);
  ...
  table.value={
    ...
    searchConfig:false,
    tableConfig: {
      url: api.url2.sys.station.equipment.list,
      showCheck: true,
      disablePage: true,
      ...
      events:{
        ...
        click_cell: async ({originData,field}) => {
          if (!field) return ;
          emit('clickCell', {type: 'equipment', data: originData});
        }
      },
      ...props.tableConfig,
      afterLoaded:()=>{
...
watch(() => [props.station], async () => {
  ...
  tableRef.value.reportConfig.getData({where:{station_id:vData.station_id,type:vData.type,pid:vData.pid,a:11}});
});
