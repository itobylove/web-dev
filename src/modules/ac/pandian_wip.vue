...
const fn = {
  createPandian: () => {
    ...
    dialogConfig.type = 'wip_create';
    dialogConfig.show = true;
  },
  sendPandian: () => {
    dialogConfig.config.title = '推送盘点表';
    dialogConfig.type = 'wip_send';
    dialogConfig.show = true;
  }
}
...
const mainReportConfig = {
  ...
  searchConfig: {
    search: [
      ...
      {
        type: 'radio', field: 'suffix', value: '-WIPSYS',
        options: {
          placeholder: '类别',
          options: [{label: '生产', value: '-WIPSYS'}, {label: '财务', value: '-WIPAC'}]
        }
      },
  ...
  tableConfig: {
    url: apiUrl.ac.pandian.wip_list,
    showCheck: false,
    disablePage: false,
  }
}
...
onMounted(async () => {
  ...
  api.get(apiUrl.ac.pandian.wip_config).then(res => {
